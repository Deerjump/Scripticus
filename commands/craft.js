...
function generateRecipe(itemCode, totalRecipe = {}) {
	...
	for (let i = 0; i < recipe.length; i++) {
		...
		const itemCode = recipe[i][0], itemQty = recipe[i][1].replace(' ', ''); 
		const itemObj = {
...
function generateRecipeText(recipe, depth=0, multiplier=1, recipeTextArr = []) {
	...
	const sorted = Object.entries(recipe).sort(([, {isRaw: a}], [, {isRaw: b}]) => b - a);
	for (let i = 0; i < sorted.length; i++) {
		const item = sorted[i][1];
		const line = `${' '.repeat(depth*3)}- ${item.name} (x${item.qty * multiplier})`;
...
module.exports = {
	...
	execute(message, args) {
		...
		const userInput = isPosInteger(lastArg) ? args.slice(0, -1).join(' ') : args.join(' ');
