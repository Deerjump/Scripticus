...
function generateRecipe(itemCode, totalRecipe = {}) {
  ...
  for (let i = 0; i < recipe.length; i++) {
    const code = recipe[i][0];
    const itemQty = recipe[i][1].replace(' ', ''); // for some reason item quantities in each cost array has a leading space in it
    const itemObj = {
      isRaw: isRawMaterial(code),
      qty: itemQty,
    };
    if (!itemObj.isRaw) {
      itemObj.recipe = generateRecipe(code);
    }
    totalRecipe[code] = itemObj;
  }
...
function generateRecipeText(
  ...
  for (const itemCode in recipe) {
    if (recipe[itemCode].isRaw) {
      const line = `${' '.repeat(depth * 3)}- ${convertCodeToDisplay(
        itemCode
      )} (x${recipe[itemCode].qty * multiplier})`;
  ...
  for (const itemCode in recipe) {
    if (!recipe[itemCode].isRaw) {
      ...
      recipeTextArr.push(line);
      generateRecipeText(
        recipe[itemCode].recipe,
        depth + 1,
        multiplier * recipe[itemCode].qty,
        recipeTextArr
      );
    }
  }
...
module.exports = {
  ...
  execute(message, args) {
    ...
    try {
      ...
      const recipeText = generateRecipeText(recipeObj, 0, itemQty).join('\n');

      const title = `Crafting recipe for ${convertToTitleCase(
        userInput
      )} (x${itemQty})`;
